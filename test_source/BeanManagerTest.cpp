...
corm::BeanManager sharedManager;
...
BOOST_AUTO_TEST_CASE(Register_bean_with_no_name) {
	...
	BOOST_REQUIRE_THROW(manager.registerBean<int&>(""), corm::InvalidBeanNameException);
}
...
BOOST_AUTO_TEST_CASE(Register_factory_scalar) {
	corm::BeanManager manager;
	manager.registerBean<DummyClass, corm::FactoryBeanCreator<DummyClass>>("factory_scalar");
...
BOOST_AUTO_TEST_CASE(Register_factory_pointer) {
	corm::BeanManager manager;
	manager.registerBean<DummyClass*, corm::FactoryBeanCreator<DummyClass*>>("factory_pointer");
...
BOOST_AUTO_TEST_CASE(Register_multiples_of_same_bean) {
	...
	BOOST_REQUIRE_THROW(manager.registerBean<int*>("multiple_of_same_bean"), corm::InvalidBeanNameException);
...
BOOST_AUTO_TEST_CASE(Retreive_bean_as_wrong_type) {
	...
	BOOST_REQUIRE_THROW(manager.getBean<int>("test_for_type_check"), corm::InvalidBeanTypeException);
}
...
#else
	BOOST_REQUIRE_THROW(manager.getBean<boost::any>("this_bean_does_not_exist"), corm::InvalidBeanNameException);
#endif
}
...
BOOST_AUTO_TEST_CASE(Check_for_Cyclic_Bean_Dependencies) {
	...
	BOOST_REQUIRE_THROW(sharedManager.getBean<A_depends_on_B*>("A_depends_on_B"), corm::BeanDependencyCycleException);
	BOOST_REQUIRE_THROW(sharedManager.getBean<B_depends_on_A*>("B_depends_on_A"), corm::BeanDependencyCycleException);
}
